...
```{r}
getSampleTibble(pseq.curateddb) |> dplyr::count(non_westernized)
```
...
euclidean_dist <- readRDS("data/rds/euclidean_dist.rds")

# Does principal coordinate analysis
euclidean_pcoa <- ecodist::pco(euclidean_dist)
#euclidean_pcoa$values

evals1 <- round(eigenvals(euclidean_pcoa)[1] / sum(eigenvals(euclidean_pcoa)) * 100, 2)
evals2 <- round(eigenvals(euclidean_pcoa)[2] / sum(eigenvals(euclidean_pcoa)) * 100, 2)
...
euclidean_plot <- ggplot(data = euclidean_pcoa_df, 
  ...
  theme_minimal() + 
  labs(x = paste0("PCo 1 ", "(" ,evals1 , "%)"),
       y = paste0("PCo 2 ", "(" ,evals2 , "%)")
...
saveRDS(bray_dist, "data/rds/bray_dist.rds")
...
```{r}
dist.to.westernized <- bray_dist_tib %>% 
  dplyr::filter(country_S2=="NLD-VEGA") %>% 
  dplyr::filter(Westernized_S1 == "YES") |>
  dplyr::group_by(S2) %>% 
  dplyr::summarise(mean.dist=mean(value, na.rm = TRUE)) |> 
  mutate(similarity="Westernized")

dist.to.non.westernized <- bray_dist_tib %>% 
  dplyr::filter(country_S2=="NLD-VEGA") %>% 
  dplyr::filter(Westernized_S1 == "NO") |>
  dplyr::group_by(S2) %>% 
  dplyr::summarise(mean.dist=mean(value, na.rm = TRUE)) |> 
  mutate(similarity="Non-Westernized")

dist.to.all <- bind_rows(dist.to.westernized,dist.to.non.westernized) 
dist.to.all |> 
  rstatix::wilcox_test(mean.dist ~ similarity, p.adjust.method = "BH")

p.dist <- dist.to.all |> 
  ggplot(aes(similarity,mean.dist)) +
    geom_boxplot(alpha=0.5,
                 width = .15,
                 outlier.shape = NA) +
    ggdist::stat_halfeye(
      aes(x = similarity, fill = similarity), 
      color = "black", size=0.8) +
    # scale_color_manual(xvar,values = fill.col) +
    # scale_fill_manual(xvar, values = fill.col) +
    labs(y="Mean similarity (1-BC) \nNLD-VEGA", x="")+
  theme_vega() +
  ggpubr::stat_compare_means(p.adjust.method ="BH") +
  scale_fill_manual("Country", values = c("Non-Westernized" = "grey50",
                                          "Westernized" = "brown3"))+
  theme(legend.position = "none") +
    theme(axis.title.x = element_blank())
ggsave("results/composition/fig/distance_to_westernized.pdf", h=3, w=5)
```

```{r}
getSampleTibble(ps.merge) |> 
  filter(country !="NLD-VEGA") |> 
  count(non_westernized)
```
...
p2 <- (p.dist | plot_spacer()) / (firm.bac.plot + bac.prev.plot) + plot_layout(heights = c(1,0.5))

p1 / p2 + 
  plot_layout(heights = c(1,2), guides = "collect") +
  plot_annotation(tag_levels = "a")
ggsave("results/composition/fig/01-Fig-01-R1.pdf", h=10, w=10)
