...
bootstrap.microbes <- function(D, Nsample = NULL, minPrev = 2, Nboot = 1000, 
  ...
  if (!is.null(ncore) && ncore > 1 ) {
    boot.which <- mclapply(boot, function(x){ 
      ...
      if (is.null(I.max))
        I.max = max(apply(D[,x], 1, 
                          function(xx) quantile(xx, probs = (1 - Prev/length(x)), na.rm = F)));
    ...
    boot.which <- lapply(boot, function(x){ 
      ...
      if (is.null(I.max))
        I.max = max(apply(D[,x], 1, 
                          function(xx) quantile(xx, probs = (1 - Prev/length(x)), na.rm = TRUE)));
      I.max = max(I.thr, I.max); # Ensure I.max > I.thr, otherwise Insty gives NA's / LL 13.8.2012
      Insty = runif(1, I.thr, I.max);
      return(core.which(D[,x], Insty, Prev))
    })
...
bootstrap.microbecount <- function(D, Nsample = NULL, minprev = 1, 
  ...
  # fulfilling prevalence criterion
  if (Nsample>1 && ncore > 1 && !is.null(ncore)) {
    boot.which=mclapply(boot,function(x){
      if (is.null(I.max)) 
        I.max=max(apply(D[,x],1, function(x){min(x[x > 0])}))
      Insty=runif(1,I.thr,I.max)
      sum(rowSums(D[,x]>Insty) >= minprev)
    }, mc.cores = ncore)
  } else if (Nsample>1 && ncore == 1 && !is.null(ncore)) {
    boot.which=lapply(boot,function(x){ 
      if (is.null(I.max)) 
        I.max=max(apply(D[,x],1,function(x){min(x[x > 0])}))
      Insty=runif(1,I.thr,I.max)
  ...
  } else if (Nsample>1 && is.null(ncore)) {
    boot.which=lapply(boot,function(x){ 
      if (is.null(I.max)) 
        I.max=max(D[,x])
      Insty=runif(1,I.thr,I.max)
  ...
  t1 <- quantile(boot.prob, probs = c(0.05, 0.5, 0.95), na.rm= FALSE)
  #t1 <- quantile(boot.prob, probs = c(.05, 0.25, 0.5, 0.75, 0.95), na.rm= FALSE)
  #t1[2] <- mean(boot.prob)
